...
#pragma omp for
  for (j = 1; j < grid_points[1]-1; j++) {
	...
	for (m = 0; m < 5; m++) {
	  ue[(i)*PROBLEM_SIZE+m] = dtemp[m];
	}
	printf("1\n");
	dtpp = 1.0 / dtemp[0];

	for (m = 1; m <= 4; m++) {
	  buf[i*PROBLEM_SIZE+m] = dtpp * dtemp[m];
	}

	printf("2\n");

	cuf[i]   = buf[i*PROBLEM_SIZE+1] * buf[i*PROBLEM_SIZE+1];
	
	buf[i*PROBLEM_SIZE+0] = cuf[i] + buf[i*PROBLEM_SIZE+2] * buf[i*PROBLEM_SIZE+2] + 
	  buf[i*PROBLEM_SIZE+3] * buf[i*PROBLEM_SIZE+3];
	
	q[i] = 0.5*(buf[i*PROBLEM_SIZE+1]*ue[(i)*PROBLEM_SIZE+1] + buf[i*PROBLEM_SIZE+2]*ue[(i)*PROBLEM_SIZE+2] +
		    buf[i*PROBLEM_SIZE+3]*ue[(i)*PROBLEM_SIZE+3]);
      }
      printf("3\n");
	...
	forcing[i][j][k][0] = forcing[i][j][k][0] -
	  tx2*(ue[ip1*PROBLEM_SIZE+1]-ue[im1*PROBLEM_SIZE+1])+
	  dx1tx1*(ue[ip1*PROBLEM_SIZE+0]-2.0*ue[(i)*PROBLEM_SIZE+0]+ue[im1*PROBLEM_SIZE+0]);

	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  tx2 * ((ue[ip1*PROBLEM_SIZE+1]*buf[ip1*PROBLEM_SIZE+1]+c2*(ue[ip1*PROBLEM_SIZE+4]-q[ip1]))-
		 (ue[im1*PROBLEM_SIZE+1]*buf[im1*PROBLEM_SIZE+1]+c2*(ue[im1*PROBLEM_SIZE+4]-q[im1])))+
	  xxcon1*(buf[ip1*PROBLEM_SIZE+1]-2.0*buf[i*PROBLEM_SIZE+1]+buf[im1*PROBLEM_SIZE+1])+
	  dx2tx1*( ue[ip1*PROBLEM_SIZE+1]-2.0* ue[(i)*PROBLEM_SIZE+1]+ ue[im1*PROBLEM_SIZE+1]);

	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  tx2 * (ue[ip1*PROBLEM_SIZE+2]*buf[ip1*PROBLEM_SIZE+1]-ue[im1*PROBLEM_SIZE+2]*buf[im1*PROBLEM_SIZE+1])+
	  xxcon2*(buf[ip1*PROBLEM_SIZE+2]-2.0*buf[i*PROBLEM_SIZE+2]+buf[im1*PROBLEM_SIZE+2])+
	  dx3tx1*( ue[ip1*PROBLEM_SIZE+2]-2.0* ue[(i)*PROBLEM_SIZE+2]+ ue[im1*PROBLEM_SIZE+2]);
                  
	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  tx2*(ue[ip1*PROBLEM_SIZE+3]*buf[ip1*PROBLEM_SIZE+1]-ue[im1*PROBLEM_SIZE+3]*buf[im1*PROBLEM_SIZE+1])+
	  xxcon2*(buf[ip1*PROBLEM_SIZE+3]-2.0*buf[i*PROBLEM_SIZE+3]+buf[im1*PROBLEM_SIZE+3])+
	  dx4tx1*( ue[ip1*PROBLEM_SIZE+3]-2.0* ue[(i)*PROBLEM_SIZE+3]+ ue[im1*PROBLEM_SIZE+3]);

	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  tx2*(buf[ip1*PROBLEM_SIZE+1]*(c1*ue[ip1*PROBLEM_SIZE+4]-c2*q[ip1])-
	       buf[im1*PROBLEM_SIZE+1]*(c1*ue[im1*PROBLEM_SIZE+4]-c2*q[im1]))+
	  0.5*xxcon3*(buf[ip1*PROBLEM_SIZE+0]-2.0*buf[i*PROBLEM_SIZE+0]+buf[im1*PROBLEM_SIZE+0])+
	  xxcon4*(cuf[ip1]-2.0*cuf[i]+cuf[im1])+
	  xxcon5*(buf[ip1*PROBLEM_SIZE+4]-2.0*buf[i*PROBLEM_SIZE+4]+buf[im1*PROBLEM_SIZE+4])+
	  dx5tx1*( ue[ip1*PROBLEM_SIZE+4]-2.0* ue[(i)*PROBLEM_SIZE+4]+ ue[im1*PROBLEM_SIZE+4]);
      }
...
c-------------------------------------------------------------------*/
	...
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (5.0*ue[(i)*PROBLEM_SIZE+m] - 4.0*ue[(i+1)*PROBLEM_SIZE+m] +ue[(i+2)*PROBLEM_SIZE+m]);
	i = 2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (-4.0*ue[(i-1)*PROBLEM_SIZE+m] + 6.0*ue[(i)*PROBLEM_SIZE+m] -
	...
	for (i = 1*3; i <= grid_points[0]-3*1-1; i++) {
	  forcing[i][j][k][m] = forcing[i][j][k][m] - dssp*
	    (ue[(i-2)*PROBLEM_SIZE+m] - 4.0*ue[(i-1)*PROBLEM_SIZE+m] +
	     6.0*ue[(i)*PROBLEM_SIZE+m] - 4.0*ue[(i+1)*PROBLEM_SIZE+m] + ue[(i+2)*PROBLEM_SIZE+m]);
	}
      }
      printf("6\n");
	...
	//i = 1*3;
      	/* forcing[i][j][k][m] = forcing[i][j][k][m] - dssp * */
      	/*   (ue[(i-2)*PROBLEM_SIZE+m] - 4.0*ue[(i-1)*PROBLEM_SIZE+m] + */
      	/*    6.0*ue[(i)*PROBLEM_SIZE+m] - 4.0*ue[(i+1)*PROBLEM_SIZE+m]); */

	printf("start i %d, m %d\n", i, m);
	//error:	printf("ue -2 %f \n", ue[(i-2)*PROBLEM_SIZE+m]);
	//error : printf("ue -1 %f \n", ue[(i-1)*PROBLEM_SIZE+m]);
	//error : printf("ue 0 %f \n", ue[(i)*PROBLEM_SIZE+m]);

	//error: printf("ue +1 %f \n", ue[(i+1)*PROBLEM_SIZE+m]);
	//return;
	double temp1 =  (ue[(i-2)*PROBLEM_SIZE+m] - 4.0*ue[(i-1)*PROBLEM_SIZE+m] +
			 6.0*ue[(i)*PROBLEM_SIZE+m] - 4.0*ue[(i+1)*PROBLEM_SIZE+m]);
	printf("temp1 %f \n", temp1);
	printf("forcing %f \n",  forcing[i][j][k][m]);
        double temp  = forcing[i][j][k][m] - dssp *
      	  (ue[(i-2)*PROBLEM_SIZE+m] - 4.0*ue[(i-1)*PROBLEM_SIZE+m] +
      	   6.0*ue[(i)*PROBLEM_SIZE+m] - 4.0*ue[(i+1)*PROBLEM_SIZE+m]);
	...
	printf("this one\n");
      	...
      	i = grid_points[0]-2;
      	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
      	  (ue[(i-2)*PROBLEM_SIZE+m] - 4.0*ue[(i-1)*PROBLEM_SIZE+m] + 5.0*ue[(i)*PROBLEM_SIZE+m]);
      }
      printf("7\n");
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	exact_solution(xi, eta, zeta, dtemp);
	for (m = 0; m < 5; m++) {
	  ue[(j)*PROBLEM_SIZE+m] = dtemp[m];
	}
                  
	dtpp = 1.0/dtemp[0];

	for (m = 1; m <= 4; m++) {
	  buf[j*PROBLEM_SIZE+m] = dtpp * dtemp[m];
	}

	cuf[j]   = buf[j*PROBLEM_SIZE+2] * buf[j*PROBLEM_SIZE+2];
	buf[j*PROBLEM_SIZE+0] = cuf[j] + buf[j*PROBLEM_SIZE+1] * buf[j*PROBLEM_SIZE+1] + 
	  buf[j*PROBLEM_SIZE+3] * buf[j*PROBLEM_SIZE+3];
	q[j] = 0.5*(buf[j*PROBLEM_SIZE+1]*ue[(j)*PROBLEM_SIZE+1] + buf[j*PROBLEM_SIZE+2]*ue[(j)*PROBLEM_SIZE+2] +
		    buf[j*PROBLEM_SIZE+3]*ue[(j)*PROBLEM_SIZE+3]);
      }
	...
	forcing[i][j][k][0] = forcing[i][j][k][0] -
	  ty2*( ue[(jp1)*PROBLEM_SIZE+2]-ue[jm1*PROBLEM_SIZE+2] )+
	  dy1ty1*(ue[(jp1)*PROBLEM_SIZE+0]-2.0*ue[(j)*PROBLEM_SIZE+0]+ue[jm1*PROBLEM_SIZE+0]);

	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  ty2*(ue[(jp1)*PROBLEM_SIZE+1]*buf[(jp1)*PROBLEM_SIZE+2]-ue[jm1*PROBLEM_SIZE+1]*buf[jm1*PROBLEM_SIZE+2])+
	  yycon2*(buf[(jp1)*PROBLEM_SIZE+1]-2.0*buf[j*PROBLEM_SIZE+1]+buf[jm1*PROBLEM_SIZE+1])+
	  dy2ty1*( ue[(jp1)*PROBLEM_SIZE+1]-2.0* ue[(j)*PROBLEM_SIZE+1]+ ue[jm1*PROBLEM_SIZE+1]);

	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  ty2*((ue[(jp1)*PROBLEM_SIZE+2]*buf[(jp1)*PROBLEM_SIZE+2]+c2*(ue[(jp1)*PROBLEM_SIZE+4]-q[jp1]))-
	       (ue[jm1*PROBLEM_SIZE+2]*buf[jm1*PROBLEM_SIZE+2]+c2*(ue[jm1*PROBLEM_SIZE+4]-q[jm1])))+
	  yycon1*(buf[(jp1)*PROBLEM_SIZE+2]-2.0*buf[j*PROBLEM_SIZE+2]+buf[jm1*PROBLEM_SIZE+2])+
	  dy3ty1*( ue[(jp1)*PROBLEM_SIZE+2]-2.0*ue[(j)*PROBLEM_SIZE+2] +ue[jm1*PROBLEM_SIZE+2]);

	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  ty2*(ue[(jp1)*PROBLEM_SIZE+3]*buf[(jp1)*PROBLEM_SIZE+2]-ue[jm1*PROBLEM_SIZE+3]*buf[jm1*PROBLEM_SIZE+2])+
	  yycon2*(buf[(jp1)*PROBLEM_SIZE+3]-2.0*buf[j*PROBLEM_SIZE+3]+buf[jm1*PROBLEM_SIZE+3])+
	  dy4ty1*( ue[(jp1)*PROBLEM_SIZE+3]-2.0*ue[(j)*PROBLEM_SIZE+3]+ ue[jm1*PROBLEM_SIZE+3]);

	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  ty2*(buf[(jp1)*PROBLEM_SIZE+2]*(c1*ue[(jp1)*PROBLEM_SIZE+4]-c2*q[jp1])-
	       buf[jm1*PROBLEM_SIZE+2]*(c1*ue[jm1*PROBLEM_SIZE+4]-c2*q[jm1]))+
	  0.5*yycon3*(buf[(jp1)*PROBLEM_SIZE+0]-2.0*buf[j*PROBLEM_SIZE+0]+
                      buf[jm1*PROBLEM_SIZE+0])+
	  yycon4*(cuf[jp1]-2.0*cuf[j]+cuf[jm1])+
	  yycon5*(buf[(jp1)*PROBLEM_SIZE+4]-2.0*buf[j*PROBLEM_SIZE+4]+buf[jm1*PROBLEM_SIZE+4])+
	  dy5ty1*(ue[(jp1)*PROBLEM_SIZE+4]-2.0*ue[(j)*PROBLEM_SIZE+4]+ue[jm1*PROBLEM_SIZE+4]);
      }
...
c-------------------------------------------------------------------*/
	...
	j = 1;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (5.0*ue[(j)*PROBLEM_SIZE+m] - 4.0*ue[(j+1)*PROBLEM_SIZE+m] +ue[(j+2)*PROBLEM_SIZE+m]);
	j = 2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (-4.0*ue[(j-1)*PROBLEM_SIZE+m] + 6.0*ue[(j)*PROBLEM_SIZE+m] -
	...
	for (j = 1*3; j <= grid_points[1]-3*1-1; j++) {
	  forcing[i][j][k][m] = forcing[i][j][k][m] - dssp*
	    (ue[(j-2)*PROBLEM_SIZE+m] - 4.0*ue[(j-1)*PROBLEM_SIZE+m] +
	     6.0*ue[(j)*PROBLEM_SIZE+m] - 4.0*ue[(j+1)*PROBLEM_SIZE+m] + ue[(j+2)*PROBLEM_SIZE+m]);
	}
      }
	...
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[(j-2)*PROBLEM_SIZE+m] - 4.0*ue[(j-1)*PROBLEM_SIZE+m] +
	   6.0*ue[(j)*PROBLEM_SIZE+m] - 4.0*ue[(j+1)*PROBLEM_SIZE+m]);
	 printf("9.22\n");
	...
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[(j-2)*PROBLEM_SIZE+m] - 4.0*ue[(j-1)*PROBLEM_SIZE+m] + 5.0*ue[(j)*PROBLEM_SIZE+m]);
          printf("9.24\n");
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	exact_solution(xi, eta, zeta, dtemp);
	for (m = 0; m < 5; m++) {
	  ue[(k)*PROBLEM_SIZE+m] = dtemp[m];
	}
        break;
	dtpp = 1.0/dtemp[0];

	for (m = 1; m <= 4; m++) {
	  buf[(k)*PROBLEM_SIZE+m] = dtpp * dtemp[m];
	}

	cuf[k]   = buf[(k)*PROBLEM_SIZE+3] * buf[(k)*PROBLEM_SIZE+3];
	buf[(k)*PROBLEM_SIZE+0] = cuf[k] + buf[(k)*PROBLEM_SIZE+1] * buf[(k)*PROBLEM_SIZE+1] + 
	  buf[(k)*PROBLEM_SIZE+2] * buf[(k)*PROBLEM_SIZE+2];
	q[k] = 0.5*(buf[(k)*PROBLEM_SIZE+1]*ue[(k)*PROBLEM_SIZE+1] + buf[(k)*PROBLEM_SIZE+2]*ue[(k)*PROBLEM_SIZE+2] +
		    buf[(k)*PROBLEM_SIZE+3]*ue[(k)*PROBLEM_SIZE+3]);
      }
	...
	forcing[i][j][k][0] = forcing[i][j][k][0] -
	  tz2*( ue[(kp1)*PROBLEM_SIZE+3]-ue[(km1)*PROBLEM_SIZE+3] )+
	  dz1tz1*(ue[(kp1)*PROBLEM_SIZE+0]-2.0*ue[(k)*PROBLEM_SIZE+0]+ue[(km1)*PROBLEM_SIZE+0]);

	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  tz2 * (ue[(kp1)*PROBLEM_SIZE+1]*buf[(kp1)*PROBLEM_SIZE+3]-ue[(km1)*PROBLEM_SIZE+1]*buf[(km1)*PROBLEM_SIZE+3])+
	  zzcon2*(buf[(kp1)*PROBLEM_SIZE+1]-2.0*buf[(k)*PROBLEM_SIZE+1]+buf[(km1)*PROBLEM_SIZE+1])+
	  dz2tz1*( ue[(kp1)*PROBLEM_SIZE+1]-2.0* ue[(k)*PROBLEM_SIZE+1]+ ue[(km1)*PROBLEM_SIZE+1]);

	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  tz2 * (ue[(kp1)*PROBLEM_SIZE+2]*buf[(kp1)*PROBLEM_SIZE+3]-ue[(km1)*PROBLEM_SIZE+2]*buf[(km1)*PROBLEM_SIZE+3])+
	  zzcon2*(buf[(kp1)*PROBLEM_SIZE+2]-2.0*buf[(k)*PROBLEM_SIZE+2]+buf[(km1)*PROBLEM_SIZE+2])+
	  dz3tz1*(ue[(kp1)*PROBLEM_SIZE+2]-2.0*ue[(k)*PROBLEM_SIZE+2]+ue[(km1)*PROBLEM_SIZE+2]);

	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  tz2 * ((ue[(kp1)*PROBLEM_SIZE+3]*buf[(kp1)*PROBLEM_SIZE+3]+c2*(ue[(kp1)*PROBLEM_SIZE+4]-q[kp1]))-
		 (ue[(km1)*PROBLEM_SIZE+3]*buf[(km1)*PROBLEM_SIZE+3]+c2*(ue[(km1)*PROBLEM_SIZE+4]-q[km1])))+
	  zzcon1*(buf[(kp1)*PROBLEM_SIZE+3]-2.0*buf[(k)*PROBLEM_SIZE+3]+buf[(km1)*PROBLEM_SIZE+3])+
	  dz4tz1*( ue[(kp1)*PROBLEM_SIZE+3]-2.0*ue[(k)*PROBLEM_SIZE+3] +ue[(km1)*PROBLEM_SIZE+3]);

	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  tz2 * (buf[(kp1)*PROBLEM_SIZE+3]*(c1*ue[(kp1)*PROBLEM_SIZE+4]-c2*q[kp1])-
		 buf[(km1)*PROBLEM_SIZE+3]*(c1*ue[(km1)*PROBLEM_SIZE+4]-c2*q[km1]))+
	  0.5*zzcon3*(buf[(kp1)*PROBLEM_SIZE+0]-2.0*buf[(k)*PROBLEM_SIZE+0]
                      +buf[(km1)*PROBLEM_SIZE+0])+
	  zzcon4*(cuf[kp1]-2.0*cuf[k]+cuf[km1])+
	  zzcon5*(buf[(kp1)*PROBLEM_SIZE+4]-2.0*buf[(k)*PROBLEM_SIZE+4]+buf[(km1)*PROBLEM_SIZE+4])+
	  dz5tz1*( ue[(kp1)*PROBLEM_SIZE+4]-2.0*ue[(k)*PROBLEM_SIZE+4]+ ue[(km1)*PROBLEM_SIZE+4]);
      }
...
c-------------------------------------------------------------------*/
	...
	k = 1;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (5.0*ue[(k)*PROBLEM_SIZE+m] - 4.0*ue[(k+1)*PROBLEM_SIZE+m] +ue[(k+2)*PROBLEM_SIZE+m]);
	k = 2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (-4.0*ue[(k-1)*PROBLEM_SIZE+m] + 6.0*ue[(k)*PROBLEM_SIZE+m] -
	...
	for (k = 1*3; k <= grid_points[2]-3*1-1; k++) {
	  forcing[i][j][k][m] = forcing[i][j][k][m] - dssp*
	    (ue[(k-2)*PROBLEM_SIZE+m] - 4.0*ue[(k-1)*PROBLEM_SIZE+m] +
	     6.0*ue[(k)*PROBLEM_SIZE+m] - 4.0*ue[(k+1)*PROBLEM_SIZE+m] + ue[(k+2)*PROBLEM_SIZE+m]);
	}
      }
	...
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[(k-2)*PROBLEM_SIZE+m] - 4.0*ue[(k-1)*PROBLEM_SIZE+m] +
	   6.0*ue[(k)*PROBLEM_SIZE+m] - 4.0*ue[(k+1)*PROBLEM_SIZE+m]);
	k = grid_points[2]-2;
	forcing[i][j][k][m] = forcing[i][j][k][m] - dssp *
	  (ue[(k-2)*PROBLEM_SIZE+m] - 4.0*ue[(k-1)*PROBLEM_SIZE+m] + 5.0*ue[(k)*PROBLEM_SIZE+m]);
      }
