...
static int program_BT(char *buf, void* priv);
...
#pragma omp for
  for (j = 1; j < grid_points[1]-1; j++) {
	...
	buf[i][0] = cuf[i] + buf[i][2] * buf[i][2] + 
	  buf[i][3] * buf[i][3];
	q[i] = 0.5*(buf[i][1]*ue[i*PROBLEM_SIZE+1] + buf[i][2]*ue[i*PROBLEM_SIZE+2] +
		    buf[i][3]*ue[i*PROBLEM_SIZE+3]);
      }
      printf("3\n");
	...
	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  tx2 * ((ue[ip1*PROBLEM_SIZE+1]*buf[ip1][1]+c2*(ue[ip1*PROBLEM_SIZE+4]-q[ip1]))-
		 (ue[im1*PROBLEM_SIZE+1]*buf[im1][1]+c2*(ue[im1*PROBLEM_SIZE+4]-q[im1])))+
	...
	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  tx2 * (ue[ip1*PROBLEM_SIZE+2]*buf[ip1][1]-ue[im1*PROBLEM_SIZE+2]*buf[im1][1])+
	...
	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  tx2*(ue[ip1*PROBLEM_SIZE+3]*buf[ip1][1]-ue[im1*PROBLEM_SIZE+3]*buf[im1][1])+
	...
	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  tx2*(buf[ip1][1]*(c1*ue[ip1*PROBLEM_SIZE+4]-c2*q[ip1])-
	       buf[im1][1]*(c1*ue[im1*PROBLEM_SIZE+4]-c2*q[im1]))+
...
c-------------------------------------------------------------------*/
      ...
      printf("access to threadprivate ue %f \n", ue[0*PROBLEM_SIZE+0]);
      for (m = 0; m < 5; m++) {
	i = 1;
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	buf[j][0] = cuf[j] + buf[j][1] * buf[j][1] + 
	  buf[j][3] * buf[j][3];
	q[j] = 0.5*(buf[j][1]*ue[j*PROBLEM_SIZE+1] + buf[j][2]*ue[j*PROBLEM_SIZE+2] +
		    buf[j][3]*ue[j*PROBLEM_SIZE+3]);
      }
	...
	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  ty2*(ue[jp1*PROBLEM_SIZE+1]*buf[jp1][2]-ue[jm1*PROBLEM_SIZE+1]*buf[jm1][2])+
	  yycon2*(buf[jp1][1]-2.0*buf[j][1]+buf[jm1][1])+
	  dy2ty1*( ue[jp1*PROBLEM_SIZE+1]-2.0* ue[j*PROBLEM_SIZE+1]+ ue[jm1*PROBLEM_SIZE+1]);

	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  ty2*((ue[jp1*PROBLEM_SIZE+2]*buf[jp1][2]+c2*(ue[jp1*PROBLEM_SIZE+4]-q[jp1]))-
	       (ue[jm1*PROBLEM_SIZE+2]*buf[jm1][2]+c2*(ue[jm1*PROBLEM_SIZE+4]-q[jm1])))+
	  yycon1*(buf[jp1][2]-2.0*buf[j][2]+buf[jm1][2])+
	  dy3ty1*( ue[jp1*PROBLEM_SIZE+2]-2.0*ue[j*PROBLEM_SIZE+2] +ue[jm1*PROBLEM_SIZE+2]);

	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  ty2*(ue[jp1*PROBLEM_SIZE+3]*buf[jp1][2]-ue[jm1*PROBLEM_SIZE+3]*buf[jm1][2])+
	  yycon2*(buf[jp1][3]-2.0*buf[j][3]+buf[jm1][3])+
	  dy4ty1*( ue[jp1*PROBLEM_SIZE+3]-2.0*ue[j*PROBLEM_SIZE+3]+ ue[jm1*PROBLEM_SIZE+3]);

	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  ty2*(buf[jp1][2]*(c1*ue[jp1*PROBLEM_SIZE+4]-c2*q[jp1])-
	       buf[jm1][2]*(c1*ue[jm1*PROBLEM_SIZE+4]-c2*q[jm1]))+
...
#pragma omp for
  for (i = 1; i < grid_points[0]-1; i++) {
	...
	buf[k][0] = cuf[k] + buf[k][1] * buf[k][1] + 
	  buf[k][2] * buf[k][2];
	q[k] = 0.5*(buf[k][1]*ue[k*PROBLEM_SIZE+1] + buf[k][2]*ue[k*PROBLEM_SIZE+2] +
		    buf[k][3]*ue[k*PROBLEM_SIZE+3]);
      }
	...
	forcing[i][j][k][1] = forcing[i][j][k][1] -
	  tz2 * (ue[kp1*PROBLEM_SIZE+1]*buf[kp1][3]-ue[km1*PROBLEM_SIZE+1]*buf[km1][3])+
	  zzcon2*(buf[kp1][1]-2.0*buf[k][1]+buf[km1][1])+
	  dz2tz1*( ue[kp1*PROBLEM_SIZE+1]-2.0* ue[k*PROBLEM_SIZE+1]+ ue[km1*PROBLEM_SIZE+1]);

	forcing[i][j][k][2] = forcing[i][j][k][2] -
	  tz2 * (ue[kp1*PROBLEM_SIZE+2]*buf[kp1][3]-ue[km1*PROBLEM_SIZE+2]*buf[km1][3])+
	  zzcon2*(buf[kp1][2]-2.0*buf[k][2]+buf[km1][2])+
	  dz3tz1*(ue[kp1*PROBLEM_SIZE+2]-2.0*ue[k*PROBLEM_SIZE+2]+ue[km1*PROBLEM_SIZE+2]);

	forcing[i][j][k][3] = forcing[i][j][k][3] -
	  tz2 * ((ue[kp1*PROBLEM_SIZE+3]*buf[kp1][3]+c2*(ue[kp1*PROBLEM_SIZE+4]-q[kp1]))-
		 (ue[km1*PROBLEM_SIZE+3]*buf[km1][3]+c2*(ue[km1*PROBLEM_SIZE+4]-q[km1])))+
	  zzcon1*(buf[kp1][3]-2.0*buf[k][3]+buf[km1][3])+
	  dz4tz1*( ue[kp1*PROBLEM_SIZE+3]-2.0*ue[k*PROBLEM_SIZE+3] +ue[km1*PROBLEM_SIZE+3]);

	forcing[i][j][k][4] = forcing[i][j][k][4] -
	  tz2 * (buf[kp1][3]*(c1*ue[kp1*PROBLEM_SIZE+4]-c2*q[kp1])-
		 buf[km1][3]*(c1*ue[km1*PROBLEM_SIZE+4]-c2*q[km1]))+
